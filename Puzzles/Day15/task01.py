...
def generate_combinations(n, total):
    ...
    candidates = range(1, total + 1)
    valid_combinations: deque[tuple] = deque()
    pbar = tqdm(total=94109400, desc= "generate combinations", bar_format='{desc}: |{bar}|{percentage:3.0f}% ({remaining}sec)')

    for combination in itertools.permutations(candidates, n):
        pbar.update(1)
...
for possibilities in all_combinations:
    ...
    temp_score_cap, temp_score_du, temp_score_fl, temp_score_te = 0, 0, 0, 0

print(f"Points of the best scoring cookies: {max(score)}")
